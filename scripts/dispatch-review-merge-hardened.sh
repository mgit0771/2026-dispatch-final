...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/dispatch-review-merge-hardened.sh --project PROJECT --target-repo OWNER/REPO --review-prompt-file /abs/path/review.md [--worker-name NAME] [--mode gate|auto] [--max-wait-sec N] [--teardown] [--scripts-dir DIR] [--dry-run]
EOF
}
...
merge_json_from_log() {
  local log_file="$1" merge_json=""

  merge_json="$(jq -c '.[]? | select(.type == "result")' "$log_file" 2>/dev/null | tail -n 1 || true)"
  if [ -z "$merge_json" ]; then
    merge_json="$(json_object_from_log "$log_file" || true)"
  fi
  [ -n "$merge_json" ] || return 1
  printf '%s\n' "$merge_json"
}

merge_result_from_log() {
  local log_file="$1" merge_result=""

  merge_result="$(jq -r '.[]? | select(.type == "result") | .result // empty' "$log_file" 2>/dev/null | tail -n 1 || true)"
  if [ -z "$merge_result" ]; then
    merge_result="$(grep -oE 'MERGE_STATUS: [A-Za-z0-9._-]+|MERGED_SHA: [A-Fa-f0-9]{7,40}' "$log_file" 2>/dev/null || true)"
  fi
  printf '%s\n' "$merge_result"
}

merged_sha_from_log() {
  local log_file="$1" merged_sha=""

  merged_sha="$(
    jq -r '.[]? | select(.type == "result") | .result // empty' "$log_file" 2>/dev/null \
      | grep -oE 'MERGED_SHA:[[:space:]]*[A-Fa-f0-9]{7,40}' \
      | sed -E 's/^MERGED_SHA:[[:space:]]*//' \
      | head -n 1 || true
  )"

  if [ -z "$merged_sha" ]; then
    merged_sha="$(grep -oE 'MERGED_SHA: [A-Fa-f0-9]{7,40}' "$log_file" 2>/dev/null | awk '{print $2}' | head -n 1 || true)"
  fi

  printf '%s\n' "$merged_sha"
}

merge_status_from_result() {
...
worker_is_running() {
  pgrep -f -- "codex exec.*-C ${PROJECT_DIR}/" >/dev/null 2>&1
}
...
phase7() {
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    log "Phase 7 (wait worker DONE): DRY-RUN, would poll codex exec and inspect ${CODEX_HEADLESS_ROOT}/${PROJECT}/${WORKER_NAME}/final-*.txt"
    WORKER_FINAL="${CODEX_HEADLESS_ROOT}/${PROJECT}/${WORKER_NAME}/final-DRY-RUN.txt"
  ...
  while :; do
    ...
    [ -n "$WORKER_FINAL" ] || die 4 "Worker process is gone but no non-empty final output was found under ${CODEX_HEADLESS_ROOT}/${PROJECT}/${WORKER_NAME}."
...
phase8() {
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    ...
    printf 'PASS: proceeding to merge\n'
    return 0
  fi
  ...
  review_json="$(json_object_from_log "$REVIEW_LOG" || true)"
  ...
  case "$REVIEW_VERDICT" in
    ...
    WARN)
      ...
      ;;
    FAIL)
      printf 'BLOCKED: verdict=FAIL hard stops merge regardless of mode\n'
      return 11
      ;;
  esac
}
...
phase9() {
  ...
  merge_json="$(merge_json_from_log "$MERGE_LOG" || true)"
  ...
  if [ -z "$merge_result" ]; then
    merge_result="$(merge_result_from_log "$MERGE_LOG")"
  fi
  ...
  MERGED_SHA="$(merged_sha_from_log "$MERGE_LOG")"
  [ -n "$MERGED_SHA" ] || die 6 "Could not parse MERGED_SHA from the merge result."
  MERGE_STATUS="$(merge_status_from_result "$merge_result")"
...
phase10() {
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    log "Phase 10 (teardown): DRY-RUN, would run ${TEARDOWN_SCRIPT} --project ${PROJECT} --force"
    return 0
  fi

  bash "$TEARDOWN_SCRIPT" --project "$PROJECT" --force >"$teardown_log" 2>&1 \
