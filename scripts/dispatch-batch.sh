...
MAX_WAIT_SEC=1800
TEARDOWN=0
SCRIPTS_DIR="/root/2026-ccc-dispatcher/scripts"
DRY_RUN=0
...
declare -A VERDICTS MERGED_SHAS RESULT_STATUS START_SECS

usage() {
  cat <<'EOF'
Usage:
  ./scripts/dispatch-batch.sh --project-prefix PREFIX --manifests-dir /abs/path --target-repo-template OWNER/REPO-{N} --review-prompt-file /abs/path/review.md [--mode gate|auto] [--max-parallel N] [--max-wait-sec N] [--scripts-dir DIR] [--teardown] [--dry-run]
EOF
}
...
ensure_target_repo() {
  ...
  [ -n "$login" ] || { ERRORS+=("Unable to resolve authenticated GitHub user for ${worker}."); return 1; }
  [ "$owner" = "$login" ] || {
    ERRORS+=("Target repo does not exist and auto-create requires OWNER=${login}: ${target_repo}")
    return 1
  }
  set +e
  ...
  set -e
  if [ "$status" -ne 0 ]; then
    ERRORS+=("Failed to create ${target_repo}: ${output}")
    return 1
  fi
...
worker_is_running() { pgrep -f -- "codex exec.*${1}" >/dev/null 2>&1; }
...
parse_review_verdicts() {
  ...
  for worker in "${REVIEWABLE_WORKERS[@]}"; do
    ...
    [ -n "$verdict" ] || { verdict="FAIL"; ERRORS+=("Could not parse verdict for PR #${num}; defaulting to FAIL."); }
...
write_merge_prompt() {
  ...
  umask 077
  {
    ...
    for worker in "${PASS_WORKERS[@]}"; do
      ...
      branch="worker/${project}-${worker}"
      printf '\nPR #%s merge steps:\n' "$num"
      printf 'cd /root/2026-loop/repo-%s\n' "$project"
...
merged_sha_for_pr() {
  ...
  if [ -z "$sha" ] && [ -d "${LOOP_ROOT}/repo-${PROJECTS[$worker]}" ]; then
    repo="${LOOP_ROOT}/repo-${PROJECTS[$worker]}"
    git -C "$repo" fetch origin --prune >/dev/null 2>&1 || true
    sha="$(git -C "$repo" log origin/main -1 --format=%H 2>/dev/null || git -C "$repo" log main -1 --format=%H 2>/dev/null || true)"
  fi
  printf '%s\n' "$sha"
}
...
phase_a() {
  ...
  [ -x "${SCRIPTS_DIR}/dispatch-review-merge.sh" ] || die 1 "Missing or not executable: ${SCRIPTS_DIR}/dispatch-review-merge.sh"
  [ -x "${SCRIPTS_DIR}/dispatch-loop.sh" ] || die 1 "Missing or not executable: ${SCRIPTS_DIR}/dispatch-loop.sh"
...
wait_f1_batch() {
  ...
  for i in "${!batch_pids_ref[@]}"; do
    ...
    if [ "$status" -eq 0 ]; then
      ...
      ERRORS+=("F1 failed for ${worker} with status ${status}. Check ${F1_LOGS[$worker]}.")
    fi
  done
    ...
    for worker in "${WORKERS[@]}"; do
      ...
      if worker_is_running "$project"; then
        if [ "$elapsed" -ge "$MAX_WAIT_SEC" ]; then
          ...
          ERRORS+=("Worker ${worker} timed out after ${MAX_WAIT_SEC}s.")
        else
          pending=$((pending + 1))
        fi
        continue
      ...
      if [ -n "$final_file" ]; then
        ...
        if wait_for_pr_metadata "$worker"; then
          ...
          ERRORS+=("Worker ${worker} completed but no PR metadata was found for ${TARGET_REPOS[$worker]}.")
        fi
      elif [ "$elapsed" -ge "$MAX_WAIT_SEC" ]; then
        RESULT_STATUS["$worker"]="crashed"
        ERRORS+=("Worker ${worker} produced no final output within ${MAX_WAIT_SEC}s.")
      else
        pending=$((pending + 1))
      fi
    done
...
run_batch_merge() {
  ...
  for worker in "${REVIEWABLE_WORKERS[@]}"; do
    ...
    sha="$(merged_sha_for_pr "$worker")"
    if [ -n "$sha" ]; then
      MERGED_SHAS["$worker"]="$sha"
      RESULT_STATUS["$worker"]="merged"
    elif [ -n "$raw_status" ] && [ "$raw_status" != "merged" ]; then
      RESULT_STATUS["$worker"]="$raw_status"
      ERRORS+=("PR #$(worker_number "$worker") reported ${raw_status} during batch merge.")
    else
      RESULT_STATUS["$worker"]="merge_failed"
      ERRORS+=("B30 recovery could not determine MERGED_SHA for PR #$(worker_number "$worker").")
    fi
  done
...
phase_d() {
  ...
  PHASES_RUN+=("D")
  [ "${#REVIEWABLE_WORKERS[@]}" -gt 0 ] || { FINAL_STATUS="crashed"; return 12; }
  if [ "$DRY_RUN" -eq 1 ]; then
  ...
  select_batch_anchor || return 5
  [ -d "${LOOP_ROOT}/repo-${BATCH_PROJECT}" ] || return 5
  id "$BATCH_USER" >/dev/null 2>&1 || return 5
  sudo -u "$BATCH_USER" env HOME="$BATCH_HOME" PATH="$BATCH_PATH" bash -lc 'command -v claude >/dev/null 2>&1' || return 5
  sudo -u "$BATCH_USER" env HOME="$BATCH_HOME" PATH="$BATCH_PATH" gh auth status --hostname github.com >/dev/null 2>&1 || return 5
  write_review_prompt
  ...
  review_status=$?
  set -e
  review_json="$(json_line_from_log "$REVIEW_LOG")"
  [ "$review_status" -eq 0 ] || { ERRORS+=("Batch review failed with status ${review_status}. Check ${REVIEW_LOG}."); return 5; }
  [ -n "$review_json" ] || { ERRORS+=("Could not find JSON output in ${REVIEW_LOG}."); return 5; }
  is_error="$(printf '%s\n' "$review_json" | jq -r '.is_error // false')"
  [ "$is_error" = "false" ] || { ERRORS+=("Batch review returned is_error=true. Check ${REVIEW_LOG}."); return 5; }
  ...
  [ -n "$REVIEW_SESSION_ID" ] || { ERRORS+=("Batch review JSON did not include session_id."); return 5; }
  [ -n "$REVIEW_RESULT" ] || { ERRORS+=("Batch review JSON did not include result."); return 5; }
  set +e
  parse_review_verdicts
  review_non_pass=$?
  set -e
...
phase_e() {
  ...
  [ -f "$TEARDOWN_SCRIPT" ] || { ERRORS+=("Teardown script not found: $TEARDOWN_SCRIPT"); return 7; }
  for worker in "${WORKERS[@]}"; do
    ...
    if bash "$TEARDOWN_SCRIPT" --project "${PROJECTS[$worker]}" --force >"$teardown_log" 2>&1; then
      ...
      ERRORS+=("Teardown failed for ${PROJECTS[$worker]}. Check ${teardown_log}.")
    fi
  done
}
