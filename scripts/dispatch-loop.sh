...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/dispatch-loop.sh --project PROJECT --manifest-file /abs/path/manifest.md --target-repo OWNER/REPO --review-prompt-file /abs/path/review.md [--worker-name NAME] [--mode gate|auto] [--create-repo] [--teardown] [--max-wait-sec N] [--scripts-dir DIR] [--dry-run]
EOF
}
...
phase_a() {
  ...
  [ -x "$SCRIPTS_DIR/dispatch-review-merge.sh" ] || die 1 error "Missing or not executable: $SCRIPTS_DIR/dispatch-review-merge.sh"
...
recover_b30() {
  local merge_log="/tmp/${PROJECT}-merge.log" merge_result="" merged_sha=""
  [ -f "$merge_log" ] || return 1
  merge_result="$(jq -r '.. | objects | .result? // empty' "$merge_log" 2>/dev/null || true)"
  merged_sha="$(printf '%s\n' "$merge_result" | grep -oE 'MERGED_SHA: [a-f0-9]+' | awk '{print $2}' | head -n 1 || true)"
  if [ -z "$merged_sha" ]; then merged_sha="$(grep -oE 'MERGED_SHA: [a-f0-9]+' "$merge_log" | awk '{print $2}' | head -n 1 || true)"; fi
  if [ -z "$merged_sha" ]; then merged_sha="$(git -C "${LOOP_ROOT}/repo-${PROJECT}" log origin/main -1 --format=%H 2>/dev/null || true)"; fi
  [ -n "$merged_sha" ] || return 1
...
worker_still_running() { pgrep -f -- "codex exec.*${PROJECT}" >/dev/null 2>&1; }
...
phase_c() {
  local f2_exit
  local -a cmd=(bash "${SCRIPTS_DIR}/dispatch-review-merge.sh" --project "$PROJECT" --target-repo "$TARGET_REPO" --review-prompt-file "$REVIEW_PROMPT_FILE" --worker-name "$WORKER_NAME" --mode "$MODE" --max-wait-sec "$MAX_WAIT_SEC")
  PHASES_RUN+=("C")
